...
#Load data raster
raster_list=glob.glob('D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/GEO/*.tif')
read=[]
...
b45test= numexpr.evaluate("(reflectance_nir / reflectance_swir1) > 0.75")
#Thin_prob
...
WaterTest = np.logical_or(np.logical_and(NDVI < 0.01, reflectance_nir < 0.11),np.logical_and(NDVI < 0.1, reflectance_nir < 0.05))

#Equation 6
PCP = numexpr.evaluate("((basic_test==True) & (whitenessTest==True)& (hazeTest==True)&(b45test==True))")
#Equation 7
clearSkyWater = numpy.logical_and(WaterTest, reflectance_swir2 < 0.03)
#Equation 12
clearLand = numpy.logical_and(numpy.logical_not(PCP), numpy.logical_not(WaterTest))
#Equation 15
SATURATION_GREEN = 1
SATURATION_RED = 2
modNdvi = numpy.where((SATURATION_GREEN != 0), 0, NDVI)
modNdsi = numpy.where((SATURATION_RED != 0), 0, NDSI)
...
clear_sky_water= 1*clearSkyWater
...
export_array(clearLand, "D:/FORESTS2020/TRAINING/PyQgis/RESULT/Landsat8/Cloud Masking/C061217/clearLand.tif")
export_array(PCP, "D:/FORESTS2020/TRAINING/PyQgis/RESULT/Landsat8/Cloud Masking/C071217/PCP.tif")
