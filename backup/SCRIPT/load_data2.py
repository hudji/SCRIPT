...
#g=band[3]
#g=band[4]
red=array(read[3], dtype=float)
nir=array(read[4], dtype=float)
check = np.logical_and ( red > 1, nir > 1 )
ndvi = np.where ( check,  (nir - red ) / ( nir + red )*1000, -999 )
geo = band.GetGeoTransform()
proj = band.GetProjection()
shape = red.shape
driver = gdal.GetDriverByName("GTiff")
dst_ds = driver.Create("D:/PROJECT/FOREST 2020/TRAINING/PyQgis/DATA/RESULT/ndvi1.tif", shape[1], shape[0], 1, gdal.GDT_Float32)
