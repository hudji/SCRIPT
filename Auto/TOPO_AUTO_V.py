...
#Load Metadata
path_f ='D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/NO_SEA/FINAL/GEO/LANDSAT/*.txt' #open file for reading
...
sample_ndvi= numexpr.evaluate("(NDVI >0.5)")
slope2= dem_dict['slope2']> 35
masking_NDVI= numexpr.evaluate("(sample_ndvi== True) & (slope2 == True)")

# kf = KFold(n_splits=10)
# for train, test in kf.split(masking_NDVI):
#     train_data = np.array(masking_NDVI)[train]
#     test_data = np.array(masking_NDVI)[test]
#train, test= train_test_split(masking_NDVI, test_size=0.33, random_state=42)
# masking_NDVI[masking_NDVI==True]=1
# masking_NDVI[masking_NDVI==False]=0
area_true= masking_NDVI.nonzero() #outputnya index row n col
a_true=area_true[0]
b_true=area_true[1]
...
for item in IC_final:
    ...
    dst_ds = driver.Create("D:/FORESTS2020/TRAINING/PyQgis/RESULT/Landsat8/TOPO/TP271217/NEW/" +item + "samp_mask2.tif", shape[1], shape[0], 1, gdal.GDT_Float32)
...
for c in IC_final:
        ...
        dfn.to_csv("D:/FORESTS2020/TRAINING/PyQgis/RESULT/Landsat8/TOPO/TP271217/NEW/sample_mask.csv", index= False)
...
export_array(IC, "D:/FORESTS2020/TRAINING/PyQgis/RESULT/Landsat8/TOPO/TP271217/IC.TIF")
