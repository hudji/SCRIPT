...
f = open('D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/NEW/mtl.txt', 'r') #open file for reading
def build_data(f):
    output = {}
...
raster_list=glob.glob('D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/NEW/*.tif')
read=[]
...
raster_list_dem=glob.glob('D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/NEW/DEM/*.tif')
read2=[]
...
# IC calculation
delta=azimuth_angle - read2[0]
IC=(cos(zenit_angle)* cos (read2[2])) + (sin(zenit_angle) * sin (read2[2]) * cos(delta))#radians

# sample
area_true= read2[1].nonzero() #outputnya index row n col
a_true=area_true[0]
b_true=area_true[1]
...
for y in read:
    ...
    IC_final[y]=read[y]-(slope[0]*(IC-cos_zenith))
"""
for y in read:
    val2=read[y]
    print  val2
    dn[y]= val2[a_true, b_true].ravel()
    IC_true= IC[a_true,b_true].ravel()
    slope = linregress(IC_true, dn[y])
    #IC_final[y] = read[y] - (slope[0] * (IC - cos_zenith))
    print slope"""
...
for item in IC_final:
    ...
    dst_ds = driver.Create("D:/FORESTS2020/TRAINING/PyQgis/DATA/Landsat8/NO SEA/FINAL/GEO/RESULT/"+item+"_.tif", shape[1], shape[0], 1, gdal.GDT_Float32)
