...
#Load data raster
raster_list2=glob.glob('D:\PROJECT\FOREST 2020\TRAINING\PyQgis\DATA\SENTINEL\Data\sentinel2\clip\*.tif')
read=[]
...
area= np.where((sample_1>=1),0,1)
area2=area.reshape(row2,col2)
area_true2= area2.nonzero() #outputnya index row n col
area_true= sample.nonzero() #outputnya index row n col
a_true=area_true[0]
b_true=area_true[1]
a_false=area_true2[0]
b_false=area_true2[1]
...
dst_ds = driver.Create("D:\PROJECT\FOREST 2020\TRAINING\SENTINEL\Result\R41017\cos\IC.tif", shape[1], shape[0], 1, gdal.GDT_Float32)
dst_ds.SetGeoTransform(geo)
dst_ds.SetProjection(proj)
dst_ds.GetRasterBand(1).WriteArray(IC)
...
#Corrected
""""
corr_2=IC_2[a_false,b_false]
corr_2F=corr_2.ravel()
corr_11=IC_11[a_false,b_false]
corr_11F=corr_11.ravel()"""

c=IC[a_true,b_true]
c_data=c.ravel()
b11=band11_data.ravel()
b8=band8_data.ravel()
...
df=pd.DataFrame({'band11cor':corr_11N, 'IC':b})
df.to_csv('D:/PROJECT/FOREST 2020/TRAINING/SENTINEL/Result/Sent/corr11n.csv', index=False)